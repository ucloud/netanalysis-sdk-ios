...
@implementation UCNetDiagnosisHelper
...
{
    if (len < (sizeof(UCIPHeader) + sizeof(UICMPPacket))) {
        return NULL;
    }
    const struct UCIPHeader *ipPtr = (const UCIPHeader *)packet;
...
{
    if (len < (sizeof(UCIPHeader) + sizeof(UICMPPacket_Tracert))) {
        return NULL;
    }
    const struct UCIPHeader *ipPtr = (const UCIPHeader *)packet;
    if ((ipPtr->versionAndHeaderLength & 0xF0) != 0x40 // IPv4
    ...
    if (len < ipHeaderLength + sizeof(UICMPPacket_Tracert)) {
        return NULL;
    }
...
{
    UICMPPacket_Tracert *packet = (UICMPPacket_Tracert *)malloc(sizeof(UICMPPacket_Tracert));
    packet->type  = ENU_U_ICMPType_EchoRequest;
    packet->code = 0;
    packet->checksum = 0;
    packet->identifier = OSSwapHostToBigInt16(identifier);
    packet->seq = OSSwapHostToBigInt16(seq);
    packet->checksum = [self in_cksumWithBuffer:packet andSize:sizeof(UICMPPacket_Tracert)];
    return packet;
}
...
{
    UICMPPacket_Tracert *icmpPacket = NULL;
    icmpPacket = (UICMPPacket_Tracert *)[self tracertICMPInPacket:packet andLen:len];
